...
function Friends() {
  ...
  const [page, setPage] = useState({size: 100,
    num: 1})
  const query = useMemo(() => {
    return {
      ...page,
  ...
  const deatailData = useMemo(() => {
    return (detailData || {}).data || []
  }, [detailData])

  const {sum=0, friends={}} = useMemo(() => {
    return (data || {}).data || {}
  }, [data])
  ...
  const renderMsg = (type, result, reContentPath, reThumPath) => {
    switch (type) {
      ...
      case '视频消息':
        return (
          <video src={reContentPath} alt="" poster={reThumPath}/>
        )
      case '文字消息':
      default:
        return result
      ...
      <div className="content">
        ...
        <div className='search-wrap'>
          <div className="search">
            <Input placeholder='请输入微信号' value={value} onChange={e => setValue(e.target.value)}></Input>
